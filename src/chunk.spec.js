  ...
  for(var x = fromX; x < toX; ++x) {
    for(var y = fromY; y < toY; ++y) {
      for(var z = fromZ; z < toZ; ++z) {
        voxels.push({position: {x: x, y: y, z: z}, value: {}});
      }
    }
  }
  return voxels;
...
describe('chunk', function() {
  it('generates a render mesh from its voxels', function() {
    var voxels = [
        {position: {x: 0, y: 0, z: 0}, value: {}}
      , {position: {x: 0, y: 0, z: 1}, value: {}}
    ];
    const g = grid.create({size: {x: 1, y: 1, z: 2}, values: voxels});

    var meshData = chunk.generate(g, grid.get, {x: 1, y: 1, z: 2}, {x: 0, y: 0, z: 0}, 1);
  ...
  it('generates a render mesh when there is voxel data missing from grid locations', function() {
    ...
    var meshData = chunk.generate(g, grid.get, {x: 1, y: 1, z: 2}, {x: 0, y: 0, z: 0});
  ...
  it('generates a render mesh from a fixed size of of the total grid', function() {
    ...
    const g = grid.create({size: {x: 32, y: 32, z: 32}, values: voxels});

    var meshData = chunk.generate(g, grid.get, {x: 16, y: 16, z: 16}, {x: 0, y: 0, z: 0});
  ...
  it('generates a render mesh from a fixed size of the total grid with a chunk offset', function() {
    ...
    var meshData = chunk.generate(g, grid.get, {x: 16, y: 16, z: 16}, {x: 1, y: 1, z: 1}, 1);
  ...
  it('builds up a list of triangles as part of its mesh data', function() {
    ...
    var meshData = chunk.generate(g, grid.get, {x: 1, y: 1, z: 1}, {x: 0, y: 0, z: 0}, 1);
